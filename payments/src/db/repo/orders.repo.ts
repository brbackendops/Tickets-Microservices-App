...
interface OrdersPayload {
  ...
  userId: number;
  price: string;
  status: OrderStatus.Created | OrderStatus.Complete | OrderStatus.Cancelled | OrderStatus.AwaitingPayment | OrderStatus.ChargeCreated | SQL<unknown> | Placeholder<string, any> | undefined;
}
...
interface OrdersUpdatePayload {
  ...
  userId: number;
  price: string;
  status: OrderStatus.Created | OrderStatus.Complete | OrderStatus.Cancelled | OrderStatus.AwaitingPayment | OrderStatus.ChargeCreated | SQL<unknown> | PgColumn<ColumnBaseConfig<ColumnDataType, string>, {}, {}> | undefined;
}
...
export class OrdersRepo {
    ...
    async create(payload: OrdersPayload): Promise<void> {
        try {
            ...
            await this.DB.insert(ordersTable)
                .values(payload);
