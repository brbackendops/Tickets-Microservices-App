    ...
    onMessage(data: { orderId: number; version: number; ticketId: number; userId: number; status: OrderStatus; price: string; }, msg: Message): void {
        try {
            ...
            ordersRepo.update(data?.orderId,data)
                .then(() => {
                    logger.info("order created from event order:created in payments successfully")
                    msg.ack()
                })
                .catch((error) => {
                    logger.info(`order failed to created from event order:created in payments : ${error}`)
                    throw error
                })
