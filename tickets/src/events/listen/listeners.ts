...
export class OrderCreatedListener extends Listener<OrderCreatedI> {
    ...
    onMessage(data: { orderId: number; ticketId: number; userId: number; status: OrderStatus; expiresAt: string; }, msg: Message): void {        
        const ticketsRepo = new TicketRepo(db)
        ticketsRepo.updateLock(data.ticketId,true)
            .then(() => {
...
export class OrderCancelledListener extends Listener<OrderCancelledI> {
    ...
    onMessage(data: { orderId: number; ticketId: number; userId: number; status: OrderStatus; }, msg: Message): void {        
        const ticketsRepo = new TicketRepo(db)
        ticketsRepo.updateLock(data.ticketId,false)
            .then(() => {
                logger.info('ticket locked successfully based on lock')
                msg.ack()
                logger.info('successfully received event from orders service')
...
export class OrderCompletionListenter extends Listener<OrderCompletionData> {
    ...
    onMessage(data: { ticketId: number; }, msg: Message): void {
        ...
        logger.info("order for making complete status is recieved in tickets service from payment service")
        ticketsRepo.updateLock(data.ticketId,false)
            .then(() => {
